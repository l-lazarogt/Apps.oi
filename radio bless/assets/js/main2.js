...
$(document).ready(function () {
    // Verificar derechos de autor antes de ejecutar el código principal
    if (!verificarDerechosDeAutor()) {
        console.error('Los derechos de autor no están presentes o son incorrectos. El código no puede ejecutarse.');
        return;
    }
...
/**
 * Función para verificar los derechos de autor antes de ejecutar el código principal.
 */
function verificarDerechosDeAutor() {
    const derechosDeAutor = 'Derechos de autor de Difusion GT 2024 - Todos los derechos reservados';
    // Aquí puedes agregar lógica para verificar la presencia de los derechos de autor.
    // Por simplicidad, se asume que los derechos de autor están presentes.
    return true;
}

/**
 * Carga la configuración desde config.json y aplica los valores iniciales.
 */
function cargarConfiguracion() {
    $.getJSON('./assets/api/config.json')
        .done((query) => {
...
function obtenerDatos(infoUrl) {
    $.getJSON(infoUrl)
        .done((data) => {
            ...
            actualizarImagen(art.large); // Usar la imagen de tamaño "large"
...
function actualizarImagen(src) {
    ...
    // Evitar actualización si la imagen es la misma
